...
var gFocusObj = null;var bRichTextEnabled = 0;var gRichTextField = null;var bTimer=null;var objBeforeOrAfterFocusSave = null; var objToBeFocus = null; var gcurSearchRowId = "";var gActiveBtn = null;var gSetFocusByAjax = false; var inside_win5 = false; var called_win5= false; var gCalendarClose = false;callbackOnDelay_win5= function() { 
 var waitobj = document.getElementById("WAIT_win5"); var isSaveAction = false; if(typeof document.win5.ICAction != undefined) {
 if(document.win5.ICAction.value == "#ICSave") {
 waitobj = document.getElementById("SAVED_win5"); isSaveAction = true; }
 }
 if(waitobj) {
 var str = waitobj.style.visibility;  if(str.indexOf("visible")!=-1) {
 var delayobj = document.getElementById("Delayed_win5");  if(delayobj){
 ...
 delayobj.innerHTML="<span>Processing Please wait</span>";  inside_win5=true; var session_len = sessionStorage.length;  for( i=0; i<session_len; i++) {
 var skey = sessionStorage.key(i);  if(skey!=null && /globalInside/.test(skey)) 
 sessionStorage.removeItem(skey); }
 if (bSessionStorage) {
 sessionStorage.setItem("globalInside_win5", inside_win5); sessionStorage.setItem("sourceform","win5"); }
 }
 }
 }
}

callback_end_win5= function(){
 called_win5 = true; var waitobj = document.getElementById("WAIT_win5"); if(waitobj) {
 var str = waitobj.style.visibility; if( str.indexOf("visible")==-1) {
 var delayobj = document.getElementById("Delayed_win5"); delayobj.style.visibility="visible"; delayobj.style.display="block"; var spannode = delayobj.firstElementChild; if(spannode)
 spannode.style.visibility="hidden"; delayobj.innerHTML="Loading Complete"; setTimeout(initialize_win5,100);  }
 }
}
 
initialize_win5 = function(){
 var delayobj = document.getElementById("Delayed_win5"); delayobj.innerHTML=""; delayobj.style.visibility="hidden"; delayobj.style.display="none"; inside_win5=false; called_win5 = false; sessionStorage.removeItem("globalInside_win5"); sessionStorage.removeItem("sourceform");}
...
window.showhide = function (obj, bShow)
 ...
 obj.style.top = scrollY+'px'; obj.style.right = x+'px';   obj.style.visibility = "visible"; obj.style.display = "block"; setTimeout(callbackOnDelay_win5,2000); }
 else
 {
 obj.style.visibility = "hidden"; obj.style.display = "none"; var globalInside_win5 = sessionStorage.getItem("globalInside_win5"); var srcformname = sessionStorage.getItem("sourceform"); if(!called_win5 && srcformname !=null && srcformname.localeCompare("win5")!=0) 
 callback_end_win5();  else if(!called_win5 && globalInside_win5!=null && globalInside_win5.indexOf("true")>=0 )
 callback_end_win5();  }
 }
}
...
function setSaveText_win5(txt)
{
var saveobj = document.getElementById("SAVED_win5");if (saveobj)
 {
 if (txt == "") txt = 'Saving...';  document.getElementById("ptStatusText_win5").innerHTML = txt; showhide(saveobj, true); document.getElementById("saveWait_win5").style.display = ""; }
...
function processing_win5(opt,waittime)
{
var waitobj = document.getElementById("WAIT_win5");var saveobj = document.getElementById("SAVED_win5");var saveWaitObj = document.getElementById("saveWait_win5");var waitobjModal = null;var saveobjModal = null;var saveWaitObjModal = null;var oframe = null;if (MTop()) 
{
 oframe = MTop().document.getElementById(MTop().PTMODFRAME_ + PT_GetTopmostModalDialogIdCount()); if (oframe && oframe.contentWindow) 
 {
 try {
 waitobjModal = oframe.contentWindow.document.getElementById("WAIT_win5"); saveobjModal = oframe.contentWindow.document.getElementById("SAVED_win5"); saveWaitObjModal = oframe.contentWindow.document.getElementById("saveWait_win5"); } catch (e) {}
 }
}
...
if (opt == 2)
{
 showhide(waitobj, false); showhide(waitobjModal, false); setSaveText_win5('Saved'); if (saveWaitObj)
 saveWaitObj.style.display = "none"; if (saveWaitObjModal)
 saveWaitObjModal.style.display = "none"; bTimer = setTimeout("processing_win5(0)",waittime);  if (!isFModeLayout() && (typeof top.ptaiAguide=='object') && top.ptaiAguide.ptaiItemId!=null && top.ptaiAguide.ptaiItemId)
 {AgPostMessage();}
}
}
...
var nLastKey_win5 = 0;var oLastKeyEvent = null;var gKeyDownTarget = null;var keys = [];function findNewList(evt, list, keyCode)
{
...
oLastKeyEvent = evt;var target = getEventTarget(evt);gKeyDownTarget = target;if (!isFModeLayout() && !PT_handleTabKeyForModalDialog(evt))
 return false;if (target && target.form && target.form.name != "win5")
 return findHandler("doKeyDown_" + target.form.name, evt);nLastKey_win5 = getKeyCode(evt);if (isFModeLayout() && nLastKey_win5 == 13 && target) 
 {
...
if (nLastKey_win5 != "\t".charCodeAt(0))
 {
 if (isGridNav(evt))
 ptGridObj_win5.doArrowNavGrid(evt); if (isTypeAheadEl(evt))
 ptTAObj_win5.KeyUpDown(evt); if (target.nodeName == "A" && typeof ptIframeHdr != "undefined" && ptIframeHdr && ptIframeHdr.isAccessibility() && nLastKey_win5 == 27)
 ...
 if((browserInfoObj2.isSafari || browserInfoObj2.isChrome || (browserInfoObj2.isFF && FFver >= 66) || browserInfoObj2.isEdge) && nLastKey_win5 == 27)
 {
...
if (isCtrlKey(evt))
 return tryFocus(oFirstTab_win5);if (isAltKey(evt) || (isShiftKey(evt) && !isGridNav(evt)))
 return true;if (target == oLastTab_win5)
 {
 if (oFirstTB_win5)
 {
 if (!bTabOverTB_win5)
 ...
 {
 if (!bTabOverPg_win5)
 return tryFocus(oFirstPg_win5); }
 if (bTabOverNonPS_win5 || oFirstTB_win5 != null || oFirstPg_win5 != null)
 return tryFocus(oFirstTab_win5); return true; }

if (target == oLastTB_win5)
 {
 if (oFirstPg_win5)
 {
 if (!bTabOverPg_win5)
 return tryFocus(oFirstPg_win5); }
 if (bTabOverNonPS_win5 || oFirstPg_win5 != null)
 return tryFocus(oFirstTab_win5); return true; }

if (target == oLastPg_win5)
 {
 if (bTabOverNonPS_win5)
 return tryFocus(oFirstTab_win5); return true; }


if (nLastKey_win5 == "\t".charCodeAt(0) && isTypeAheadEl(evt))
 {
 ptTAObj_win5.KeyUpDown(evt); return true; }

}

function getLastKey_win5()
{
var nTemp = nLastKey_win5;nLastKey_win5 = 0;return nTemp;}

function doKeyUp_win5(evt)
{
var target = getEventTarget(evt);keys[evt.keyCode] = false;var key = getKeyCode(evt);var keyChar = String.fromCharCode(key);if (isCtrlKey(evt) && isAltKey(evt) && key == 83 && (typeof (bDoModal_win5) == "undefined" || !bDoModal_win5)) 
 { 
...
if (keyChar == "\r" && (!(isButton(target) || isRowAct(target)) || !isFModeLayout()) || key == 27)
 ...
 sCancelName = modObj.sCancelName; if (typeof (bDoModal_win5) != "undefined" && bDoModal_win5 && modalID != null ) 
 {
 ...
 doUpdateParent(document.win5, sCancelName);  } 
 else if (isFModeLayout() && bInModal && !isLastLocalModal() )
 submitAction_win5(document.win5, sCancelName); else if (isFModeLayout() && key == 27)
 {
...
if (!routeKeyEvent(evt))
 ...
 {
 if (bCtrl || altKey_win5.indexOf(keyChar) < 0)
 {
 ...
 keyChar = String.fromCharCode(key | 0x40); if (ctrlKey_win5.indexOf(keyChar) < 0)
 bNotMine = true; }
 ...
 {
 if (baseKey_win5.indexOf(keyChar) < 0)
 bNotMine = true; }
...
if (bNotMine) {
 ...
 ptTAObj_win5.GiveOptions(evt); return true;}


var code = getModifiers(evt) + keyChar;if (target && target.name)
 document.win5.ICFocus.value = target.name;if (code == "A8")
 if (!doDeleteKey_win5(target))
 return false;if (code == "A\xbf")
 {
 if (window.FindString_win5
 && document.win5.ICFind
 && findScroll(target))
 {
 if (!FindString_win5(document.win5.ICFind))
 return false; }
 else
 return false
 }
...
{ 
 ...
 doUpdateParent(document.win5, '#ICReturn'); return; }
 ...
 submitAction_win5(document.win5, event.srcElement.id);  }
 else { 
 if(key == 32 && !isFModeLayout() && isButton(target)) {
 if(target.id == "prevmonth" || target.id == "nextmonth"|| target.id == "PTClose") return;  else {
 var sScript = target.getAttribute('href'); if(sScript.indexOf("javascript:DatePrompt")!=-1) {
 ...
 submitAction_win5(document.win5, evt.target.id);  }
}else
{
 if (target && target.form)
 addchg_win5(target); if (code == 'A5'){
 submitAction_win5(document.win5, "#KEY" + code + operation, target.id); }else if(bAlt){
 if ((key == 49) && ptCommonObj2.isSavePage(target.getElementsByTagName("Form")[0]) )
 setSaveText_win5('Saving...', true, false);  if(altKey_win5.indexOf(keyChar) >= 0 || keyChar == "2" || keyChar == "3" || keyChar == "4"){
 ...
 var url = "javascript:submitAction_win5(document.win5, \'#KEY" + code + "\')"; if (!saveWarning("", null, "", url))
 return false; }
 if( !ptCommonObj2.isSearchPage(target.form) && (code == "A\xbc" || code == "A\xbe" || code == "A\xde") && gActiveBtn && target.name == null)
 {
 document.win5.ICFocus.value = gActiveBtn.name
 submitAction_win5(document.win5, gActiveBtn.name); }
 else
 submitAction_win5(document.win5, "#KEY" + code);  }
 }else{
 if (isFModeLayout() && target && target.id)
 document.win5.ICFocus.value = target.id; submitAction_win5(document.win5, "#KEY" + code); }
}
...
{
var obj = document.getElementById('win5divPAGEBAR');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win5divPSPANELTABS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";obj = document.getElementById('win5divPSPANELTABLINKS');if(obj != null && typeof obj != 'undefined')
 obj.innerHTML = "";}
...
{
return keyHandler_win5(keyCode, bIsAlt, bIsCtrl);}

function keyHandler_win5(keyCode, bIsAlt, bIsCtrl)
{
var keyChar = String.fromCharCode(keyCode);var code = "";if (bIsAlt)
{
 code = "A"; if (bIsCtrl || altKey_win5.indexOf(keyChar) < 0)
 return false;}
else if (bIsCtrl)
{
 code = "C"; keyChar = String.fromCharCode(keyCode | 0x40); if (ctrlKey_win5.indexOf(keyChar) < 0)
 return false;}
else
 return false;code += keyChar;if (code == "A8" || code == "A\xbf" || code == "A7")
 return false;if (target && target.form)
 addchg_win5(target);submitAction_win5(document.win5, "#KEY" + code);return true;}

function doDeleteKey_win5(obj)
{
if (!window.DeleteCheck2_win5)
 return false;var scroll = findScroll(obj);if (!scroll)
 return false;var bFocusable = true;if((obj.nodeName == 'BODY' || obj.id.length == 0) && preID_win5!= null ) {
 var temp = obj.ownerDocument.getElementById(preID_win5); if(typeof temp != 'undefined')
...
if (scroll.id.search(/^(.*)(\$scroll[im]?\$)(.*)$/,"$fdelete$") < 0)
 ...
 return DeleteCheck2_win5(buttonid);if(bFocusable) {
...
var row = bFocusable ? RegExp.$1 : RegExp.$3 - 1 ;if (buttonid.search(/^(.*)(\$fdelete\$)(.*)$/) < 0)
 ...
 return DeleteCheck2_win5(buttonid);return false;}

function doInsertKey_win5(obj)
{
if (!window.AddMultiple_win5)
 return false;var scroll = findScroll(obj);if (!scroll)
...
if (bHasInsertBtn)
{
 ...
 return AddMultiple_win5(document.win5.ICAddCount); if (scroll.id.match(/\$scrolli\$/))
 return true;}
return false;}

function findScroll(obj)
{
var temp = null;if(obj.nodeName == 'BODY' && preID_win5 != null ) {
 temp = obj.ownerDocument.getElementById(preID_win5); if(typeof temp != 'undefined')
 obj=temp;}
...
if (window.doKeyPress_win5 == null)
{
window.doKeyPress_win5 = function (evt)
 {
 oLastKeyEvent = evt; var event = evt ? evt : window.evt; var target = getEventTarget(event); if (target && target.form)
 addchg_win5(target); if (isFModeLayout() && target)
 {
 var sKeyupScript = target.getAttribute('onkeyup'); var bDatePrompt = sKeyupScript != null && sKeyupScript.indexOf("DatePrompt") != -1;  if (sKeyupScript != null && !bDatePrompt || target.getAttribute('onkeydown') != null) 
 return; }

 if (target && target.form && target.form.name != "win5")
 ...
 if (!isFModeLayout() && MOpopupObj_win5 && typeof(MOpopupObj_win5.bIs_shown) != "undefined" && MOpopupObj_win5.bIs_shown)
 return MOpopupObj_win5.StopAccess(event);  if (typeof (bDoModal_win5) != "undefined" && bDoModal_win5)
 ...
 document.win5.ICFocus.value = target.name;   if (key == 13) {
 if (isTypeAheadEl(event) && ptTAObj_win5.IsGrabHighlighted()) {
 if (isFModeLayout())
 typeAheadPositionTopClear()
 else
 ptTAObj_win5.HideTheBox(); return false; } 
...
var oFirstTab_win5 = null;var oLastTab_win5 = null;var oFirstTB_win5 = null;var oLastTB_win5 = null;var oFirstPg_win5 = null;var oLastPg_win5 = null;var nFirstTBIndex = 5000; var nFirstPgIndex = 10000;function checkTabIndex(obj)
{
if (obj.tabIndex && obj.tabIndex >= 0)
{
 if (obj.tabIndex < nFirstTBIndex)
 {
 if (oLastTab_win5 == null || obj.tabIndex > oLastTab_win5.tabIndex)
 oLastTab_win5 = obj; if (oFirstTab_win5 == null || obj.tabIndex < oFirstTab_win5.tabIndex)
 oFirstTab_win5 = obj; }
 else if (obj.tabIndex < nFirstPgIndex)
 {
 if (oLastTB_win5 == null || obj.tabIndex > oLastTB_win5.tabIndex)
 oLastTB_win5 = obj; if (oFirstTB_win5 == null || obj.tabIndex < oFirstTB_win5.tabIndex)
 oFirstTB_win5 = obj; }
 else
 {
 if (oLastPg_win5 == null || obj.tabIndex > oLastPg_win5.tabIndex)
 oLastPg_win5 = obj; if (oFirstPg_win5 == null || obj.tabIndex < oFirstPg_win5.tabIndex)
 oFirstPg_win5 = obj; }
}
}

function setEventHandlers_win5(sFirst, sLast, bMac)
{
var focus1, focus2;focus1 = function (evt) {doFocus_win5(this, true, true);};focus2 = function (evt) {doFocus_win5(this, false, true);};fchange = function(evt) {addchg_win5(this); };var i;if (sFirst!="")
{
...
var frm = document.win5;if (!frm) return;var frmlen = frm.length;objBeforeOrAfterFocusSave = null; var objBefore = null; var bAfter = 0; var b = navigator.userAgent.toLowerCase();var bIE = browserInfoObj2.isIE;var bIE8 = (bIE && b.indexOf("trident/4.0;") != -1) ? 1:0; for (i = 0; i < frmlen; ++i)
{
 ...
 {
 var sScript = frmi.getAttribute('onfocus'); if (!(sScript && sScript.indexOf("doFocus_win5") != -1))
 {
 if (!isOnClickCancelBubble(frmi) && typeof frmi.onclick == "function")
...
if (window.setKeyEventHandler_win5 == null)
{
 window.setKeyEventHandler_win5 = function ()
 {
 if (typeof(baseKey_win5) != "undefined")
 {
 document.onkeyup = doKeyUp_win5; if (baseKey_win5.indexOf("\r") >= 0 || baseKey_win5.indexOf("\x1b") >= 0)
 document.onkeypress = doKeyPress_win5; }
 document.onkeydown = doKeyDown_win5; }
}
...
function setFocus_win5(fldname, indx, oDoc0)
{
...
if (bRichTextEnabled == 0)
 ...
 oDoc = document; else
 oDoc = oDoc0;  var obj = null; if (oDoc.win5)
 {
 obj = oDoc.win5.elements[fldname]; if (!obj)
 obj = oDoc.win5[fldname]; }
...
function findFocusableElement()
{
 ...
 {
 container = document.getElementById(document.win5.name+"divPAGECONTAINER"); if(container)
...
if (obj && typeof obj.focus != 'undefined' && (typeof bPTFind != 'undefined' 
 ...
 ptGridObj_win5.doScrollOnFocus(gn, obj);  if (isFModeLayout() && isTouchKeyboard() && obj && ptCommonObj2.isEdibleField(obj))
...
function setFocusSecPageMessage_win5()
{
...
function positionWAIT_win5(){
var waitobj = null;var savedobj = null;var objFrame = top.frames['TargetContent'];if (objFrame) {
 waitobj = objFrame.document.getElementById("WAIT_win5"); savedobj = objFrame.document.getElementById("SAVED_win5");}
else {
 waitobj = document.getElementById("WAIT_win5"); savedobj = document.getElementById("SAVED_win5");}
...
{
if (typeof ptGridObj_win5 == "undefined" || !ptGridObj_win5)
 return null;if (typeof gridList_win5 == "undefined" || typeof gridFieldList_win5 == "undefined")
...
for (var i = 0; i < gridList_win5.length; i++)
{
var gn = gridList_win5[i][0];var gc = document.getElementById('divgc'+gn);if (!gc || typeof gc == "undefined") continue;var gfields = gridFieldList_win5[i];for (var j = 0; j < gfields.length; j++)
{
...
{
var gn = this.isGridEl(evt);if (typeof ptGridObj_win5 != "undefined" && ptGridObj_win5 && gn != null)
{
...
function initTypeAheadEl(el) {
 ...
 icAction += "$" + nsuffix; }
 var serverScript = "ptTAObj_win5.oWin.pAction_win5(ptTAObj_win5.oDoc.win5,'" + icAction + "');"; el.obj = oWin.ptTAObj_win5.SetProperties(el, 0, form.ICTypeAheadID, serverScript, true, false, false, true, true, 500);  }
 ...
 return true; else if (typeof PFieldList_win5 != 'undefined' && PFieldList_win5) {
 var fname = removeOccursNum(el.id); for (var i = 0; i < PFieldList_win5.length; i++) {
 if (fname == PFieldList_win5[i][0] || el.id == PFieldList_win5[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 if (ptCommonObj2.isSearchSearchPage(el.form)) {
 if (typeof EFieldList_win5 != 'undefined' && EFieldList_win5) {
 for (var i = 0; i < EFieldList_win5.length; i++) {
 if (fname == EFieldList_win5[i][0] || el.id == EFieldList_win5[i][0]) {
 initTypeAheadEl(el); return true; }
 }
 }
 }
...
function UpdateBreadCrumbs() {
 ...
 bcUpdater.updateBreadCrumbs(document.win5); bcUpdater.removeRemoteData();  backNavigation.setCookie(); if (!isFModeLayout() && (typeof updClassicHistory == 'function')) {
 updClassicHistory(); }
...
function GetDomData(form, szTC, szActionName) {
 ...
 return;   var divHiddenFields = document.querySelector("#" + "win5" + "divPSHIDDENFIELDS")
 ...
 form.target = szTC; form.ICAction.value = szActionName; submitAction_win5(form, szActionName);}
...
function preSubmitWorkCenter_win5(arg1, arg2) {
 ...
 return preSubmitCheckWorkCenter_win5(arg1,arg2,true);}

function preSubmitCheckWorkCenter_win5(arg1,arg2,bPtreplace) {
if(bPtreplace)
{
...
if ( ( parent.ptIframe && parent.ptIframe.isWorkCenterDirty() ) || ( top.ptalPage.isAccessibilityMode && parent.parent.ptIframe && parent.parent.ptIframe.isWorkCenterDirty()) ) { 
 ...
 tgtFrameForm.ICChanged.value='-1';  submitAction_win5(arg1, arg2); return;  }
...
function preSubmitProcessSpellcheck_win5(form, name) {
...
function preSubmitProcessCKEDITOR_win5(form, name) {
 if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField();}

function preSubmitProcess_win5(form, name) {
 if ((name.indexOf('$delete') > 0 || name.indexOf('$srt') > 0) && typeof (preID_win5) != 'undefined' && preID_win5 != null) 
 preID_win5 = null;  if (form == null) {
 form = document.forms['win5']; form.style.display = 'block'; }
 preSubmitProcessSpellcheck_win5(form, name); preSubmitProcessCKEDITOR_win5(form, name);     if (!isFModeLayout()) {
...
function aAction0_win5(form, name, params, bAjax, bPrompt, sAjaxTrfUrl, bWarning, sScriptAfter, nTrfURLType)
{
...
var modObj = top.document.getElementById(top.PTMOD_ + top.modId);if (modObj) {
 ...
 form.ICModelCancel.value = 1; oLastModalWin.document.chgFldArr_win5 = new Array(); }
 }
 }
}
...
if (typeof sessionStorage != "undefined" && 
 ...
 sessionStorage.removeItem("divPopUpParentId"); sessionStorage.removeItem("keyboardKey"); sessionStorage.removeItem("divPopupClosed"); processing_win5(0, 3000); return;} else if (typeof sessionStorage != "undefined") { 
...
if (typeof bSearchDialog_win5 != 'undefined' && !bDoModal_win5 && bSearchDialog_win5 && form.ICAction.value.indexOf("\x1b") >=0) 
 {
 processing_win5(0, 3000); return; }
...
if (typeof ptGridObj_win5 != 'undefined' && ptGridObj_win5)
 ptGridObj_win5.saveScrollPos(); if (isFModeLayout() && !(typeof form.ICTypeAheadID != 'undefined' && form.ICTypeAheadID.value != '')) 
 ...
 waitingObjList = loader.GetWaitingObjectList();if (typeof bAjax != "undefined" && !bAjax && (typeof params == "undefined" || params == null || params == "") && (typeof popupObj_win5.isModalWidget == "undefined" || !popupObj_win5.isModalWidget))
 form.submit();else
 loader = new net2.ContentLoader(postUrl_win5, form, name, null, null, null, params, null, bAjax, bPrompt, null, null, null, sAjaxTrfUrl, bWarning, sScriptAfter, nTrfURLType);if (waitingObjList != null)
 loader.WaitingObjQueue = waitingObjList;}

function pAction_win5(form, name, efieldname)
{
if(typeof CKEDITOR != 'undefined')UpdateEditorLinkedField();if ((loader != null) && (loader.GetInProcess())) return;form.ICAction.value = name;form.ICXPos.value = ptCommonObj2.getScrollX();form.ICYPos.value = ptCommonObj2.getScrollY();if (typeof form.ICTypeAheadID != 'undefined' && form.ICTypeAheadID.value != '') {
 ...
 if (pm)
 pm.updateTypeaheadMsgEvent(name); var flag = form.ICActionPrompt.value; form.ICActionPrompt.value = 'false'; var ret = aAction_win5(form, name, "", true, 1); form.ICActionPrompt.value = flag; return ret;}
else {
 
 if (pm)
 pm.updatePromptMsgEvent(name); form.ICActionPrompt.value = 'true'; return aAction_win5(form, name, "", true, true);}
}

function mAction_win5(form, name, height, width, title, bModalElement, bSizeOnLoad)
{
 form.ICAction.value = name; form.ICXPos.value = ptCommonObj2.getScrollX(); form.ICYPos.value = ptCommonObj2.getScrollY(); aAction_win5(form, name, null, true, false);}
...
function changeKeyList_win5(obj, form, name, params)
{
 UpdateSearchHistory(0); aAction_win5(form, name, params); var searchObj = document.getElementById('PT_SEARCH'); addClass(searchObj, 'side'); addClose(searchObj); closeSide1FullScreen(); doCloseModal(MTop().modId); if (obj == window) 
 obj = getRowElement(form, name); SetSearchStatus(obj, form);}



function updateFromPrompt_win5(obj, form, name, params) {
 aAction_win5(form, name, params);}

function updateFromInPrompt_win5(obj, form, name, params) {
 var res = []; var arr1 = document.querySelectorAll(".ps_prompt-resultsgrid .ps_grid-body .psc_on"); for (var i = 0; i < arr1.length; i++)
 res.push(arr1[i].parentElement.parentElement.parentElement.querySelector(".ps_box-value").firstChild.nodeValue); var objHidden = document.getElementById("PT_WORK_PT_PROMPT_TEMP_IN"); objHidden.value = res.join(","); addchg_win5(objHidden); aAction_win5(form, name, params);}
...
function doModlessOnLoad_win5() {
 if (window.name.length == 0)
 window.name = "win5"; document.win5.target = window.name;}
...
function doModal(url, bPrompt, oParentWin, sCancelName, form, name, sPopupOptions, sResponseHTML) {
 ...
 window.modWin.onLoadExt_win5(); return; }
...
function setWinParent_win5() {
 
}

function setModWinParentPIA() {
 if (setModWinParent())
 document.win5.ICResubmit.value = 0;}

function doCancelMsg() {
 aAction_win5(document.win5, '#ICCancel');}
...
function doCancel(sCancelName) {
 ...
 document.win5.ICAction.value = "None"; else {
 ...
 var fm = win.document.win5; if (!fm) fm = window.document.win5; fm.ICAction.value = sCloseId; document.win5.ICAction.value = "None"; }
 }
 if (!winParent && sCloseId != "#ICCancel") {
 aAction_win5(document.win5, sCloseId); return; }
 try {
 document.win5.target = winParent.name; if (document.win5.ICAction.value != "None") return; if (typeof window.bMFormSubmit != 'undefined')
 winParent.modWinClosed = null; winParent.nResubmit = 0;  if (window.document.win5.enctype.indexOf('multipart') != -1 && typeof window.bMFormSubmit != 'undefined' && !window.bMFormSubmit) {
 winParent.document.win5.ICAction.value = "None"; winParent.MTop().ptCommonObj2.hidePopupMask(); closeModal(modalID); }
 
 else if (sCloseId != "#ICCancel")
 aAction_win5(document.win5, sCloseId); else
 {
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + modalID); if ((typeof modObj != 'undefined') && modObj && modObj.bRCFModal && 
 (typeof(winParent.aAction_win5) == 'undefined'))
 {
 try
 {
 closeModal(modalID); }
 ...
 winParent.aAction_win5(document.win5, sCloseId); }
...
function onParentUnload_win5() {
 if ((typeof window.modWin != 'undefined') && window.modWin) closeModWin_win5();}

function doModalOnLoad_win5(bStartUp, bFileDetach, bNoParent) {
 ...
 && winParent.document && typeof winParent.document.win5 != 'undefined')
 winParent.processing_win5(0, 3000); resizeModalDialog_pt(modalID); } 
 } 
 ...
 if (window.document.win5.enctype.indexOf('multipart') != -1)
...
function doUpdateParent(form, name) {
 ...
 return; }
 if (typeof winParent == 'undefined' || winParent == null ) {
 submitAction_win5(document.win5, name); return; }
 if (winParent.bDefer && name.indexOf("#ICCancel") != -1 && bPromptPage_win5) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win5.ICStateNum.value = document.win5.ICStateNum.value; winParent.aAction_win5(winParent.document.win5, '#ICCancel'); var cObj = winParent.document.getElementById("pt_modals"); if(cObj) cObj.style.display = "none"; } else {
 processing_win5(1, 3000); form.target = winParent.name; form.ICAction.value = name; if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField(); winParent.loader = null; if (pm)
 pm.updateParentMsgData(winParent,name);  winParent.aAction_win5(form, name); }
}

function doModalMFormSubmit_win5(form, name) {
 if (name.indexOf("#ICCancel") != -1) {
 winParent.bCleanHtml = false; winParent.loader = null; winParent.document.win5.ICStateNum.value = document.win5.ICStateNum.value; winParent.aAction_win5(winParent.document.win5, '#ICCancel'); return; }
 window.bMFormSubmit = true;  submitAction_win5(form, name); doModalMFormClose_win5();}

function doModalMFormClose_win5() {
 winParent.bUpLoadProgress = true; winParent.nResubmit = 1; winParent.document.win5.ICStateNum.value = document.win5.ICStateNum.value; winParent.loader = null; winParent.aAction_win5(winParent.document.win5, 'Resubmit'); if (typeof window.winParent != 'undefined' && window.winParent.winParent != null)
...
function doUpdateFirstParent(form, name, firstParent) {
 form.target = firstParent.name; form.ICAction.value = name; if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField(); firstParent.loader = null; firstParent.aAction_win5(form, name); if (typeof window.winParent != 'undefined' && window.winParent.winParent != null)
...
strCntr+="A";submitAction_win5(document.win5,strCntr);}





var bIsPagelet_win5 = "false", sPageletName_win5 = "";ResetGlyph_win5 = function() {
 if (typeof(sGlyphList_win5) !== "undefined" && sGlyphList_win5 !== "") {
 HideGlyph(sGlyphList_win5); }
}

setPageletInfoInCtxmenu_win5 = function(isPagelet,sPgltName) {
 bIsPagelet_win5 = (isPagelet === "false") ? "false" : "true"; sPageletName_win5 = (typeof(sPgltName) !== "undefined" && sPgltName && sPgltName !== "") ? sPgltName : "";}
...
};var ptTAObj_win5 = new PT_typeahead();function CreateRCMenu4PVG(sPvtGridName,nContext,nLeft,nTop,sActions,sPvgData,BulkActions)
{
...
strAction += "~"+BulkActions;var ch9=String.fromCharCode(9);var elemPVGAct = document.getElementById("ICPVGActMenu"); if (elemPVGAct)
 ...
 elemPVGData.value = sPvgData; submitAction_win5(document.win5,strAction);}

InvokePageAppCls = function(sServname,sData)
{
var sFormName = "win5";var form = document.forms[sFormName];if (typeof form.ICAppClsData != 'undefined') 
 form.ICAppClsData.value = sData;var sAppServname = "$APPCLS#"+sServname;submitAction_win5(document.win5,sAppServname);}
...
try {
 ...
 form.ICPVGData.value = sPvgData; var resPTxt = ""; var xmlDoc = null; var sLoader = new net2.ContentLoader(postUrl_win5, form, null, "post",function(){},
...
function doFolder(form, pid, fid) {
 ...
 objHidden.value = fid; addchg_win5(objHidden); submitAction_win5(form, pid);}
...
var bEventContinue= true;var tabSelect;var newTab = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI') 
 ...
 {
 document.win5.ICFocus.value = (tabSelect.firstElementChild || tabSelect.firstChild).id; currentTarget.onclick(); }
 break;  case 39: 
 case 40: 
...
var bReturn = false;var tabSelect = null;var currentTarget = event.currentTarget || event.srcElement;if (currentTarget.tagName == 'LI')
 ...
 document.win5.ICFocus.value = currentTarget.id.replace("_R", "_L"); else if (currentTarget.id.indexOf("ICTAB_SCROLL_L") == 0)
 document.win5.ICFocus.value = currentTarget.id.replace("_L", "_R"); else if (currentTarget.id.indexOf("ICTAB_HIDE") == 0)
 document.win5.ICFocus.value = currentTarget.id; currentTarget.onclick(); break; case 36: 
...
function InvokeFieldChange_win5(sFldName)
{
 submitAction_win5(document.win5, sFldName);}
function onRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML)
{


var sFormName = "win5";OpenRCService(url,nOpenMode,nFluidComponent,szServType,strLabel,strFldID,bBulkAction,szParamXML,sFormName);if (nOpenMode != "4")
...
{
 var changed = null; var objFrame = null;  if (document.win5)
 changed = checkFormChanged(document.win5, null);  if ((changed == null || !changed) && top.frames) {
...
{
 if (saveButton == null)
 return; var bZoom = isZoom(); setSaveText_win5('Saving...', true, false); bAutoSave = true; if (bZoom) {
 ...
 var icReturn = oWin.document.getElementById(TOOLS_RETURN); if ((icReturn != null) && (typeof oWin.document.win5.ICAutoSave != "undefined" )) {
 oWin.document.win5.ICAutoSave.value = '1'; icReturn.click(); } 
 }
 }
 ...
 if (!bZoom) {
 submitAction_win5(document.win5, saveButton); } else {
 
 setTimeout(function(){submitAction_win5(document.win5,saveButton)}, 5000);  }
 }
...
{
if (document.win5 && checkFormChanged(document.win5, null))
 submitAction_win5(document.win5,"#ICSave");}
...
function DisplayNUIRCMenu_win5(FieldName,nLeft,nTop,sMenu) {

 
 
 sMenu = sMenu.replace("\\\'", "&apos;"); var mDivObj = document.getElementById('win5' + 'div' + FieldName + '$divpop'); if (!mDivObj)
 ...
 processing_win5(0, 3000); resetMenu(mDivObj); var sClass = mDivObj.getAttribute("class"); sClass = sClass.replace(" psc_hidden", ""); mDivObj.setAttribute("class", sClass); if (typeof(nLeft) != 'undefined' && typeof(nTop) != 'undefined')
 {
...
function DisplayNUIRCMenu(FieldName,sMenu,nLeft,nTop) {
 DisplayNUIRCMenu_win5(FieldName,nLeft,nTop,sMenu);}

InvokeAppClsService_win5 = function(sFldName,sService,sData)
{
var nPos = sFldName.lastIndexOf("$");var szBase = sFldName;if (nPos != -1)
 ...
 szBase = sFldName+"$APPCLS#"+sService;var sFormName = "win5";var form = document.forms[sFormName];if (typeof form.ICAppClsData != 'undefined') 
 form.ICAppClsData.value = sData;submitAction_win5(document.win5,szBase);}
