 ...
 xshowNoMatchMessage,xuseTimeout,xtheVisibleTime)
{
 ...
 if (isFModeLayout()) {
 var mObj = ptTAObj_win5.oDoc.getElementById(el.id +'$typeahead'); if(mObj != null && typeof mObj != "undefined"){
 ...
 ptTAObj_win5.bLostFocus = true; if(this.obj.useTimeout) {
 ptTAObj_win5.StartTimeout(); }
 }
 ...
 if (typeof ptTAObj_win5.oWin == 'undefined') 
 return; this.bTabPressed = false; this.bKeyUp = false; this.bKeyDown = false; var intKey=-1; ptTAObj_win5.bLostFocus = false; if (this.theTextBox)
 addchg_win5(this.theTextBox); if (ptTAObj_win5.oWin.event) 
 intKey=ptTAObj_win5.oWin.event.keyCode; else
 intKey = e.which; if (intKey == this.CTRL) 
 return true; if (intKey == this.ALT) { 
 ptTAObj_win5.EraseDelayTime();  return true; }
 if(intKey==this.UPARROW){
 this.bKeyUp = true; ptTAObj_win5.MoveHighlight(-1); ptTAObj_win5.GrabHighlighted(); return true; }
 else if(intKey==this.DOWNARROW){
 this.bKeyDown = true; ptTAObj_win5.MoveHighlight(1); ptTAObj_win5.GrabHighlighted(); return true; }
 ...
 if (typeof ptTAObj_win5.oWin == 'undefined') 
 return; this.PositionTop(getEventTarget(e)); var intKey=-1; if (ptTAObj_win5.oWin.event) { 
 intKey=ptTAObj_win5.oWin.event.keyCode; this.theTextBox=ptTAObj_win5.oWin.event.srcElement; }
 else{
 ...
 if (ptTAObj_win5.CancelTypeAhead(this.theTextBox)) return false;  if (intKey == this.BACKSPACE || this.lastIntKey != this.BACKSPACE) {
 ptTAObj_win5.EraseDelayTimeout(); }
 ...
 ptTAObj_win5.resetTextBox(this.theTextBox); if(this.theTextBox.value.length==0&&!this.isOpera){ 
 this.InitArrOptions(); ptTAObj_win5.HideTheBox(); ptTAObj_win5.strLastValue=''; this.cleanFluidCache(this.theTextBox); }
 if(this.objLastActive==this.theTextBox){
 if(intKey==this.ENTER){ 
 ptTAObj_win5.GrabHighlighted(); this.theTextBox.blur(); }
 } 
 if (this.theTextBox.value.length==0) 
 ptTAObj_win5.HideTheBox(); else {
 if ( ptTAObj_win5.bInProcess || (this.objLastActive!=this.theTextBox) ||
 ((this.theTextBox.value.length!=0) && (this.theTextBox.value.length > ptTAObj_win5.strLastValue.length)) ||
 (this.theTextBox.value.indexOf(ptTAObj_win5.strLastValue)!=0) ||
 (this.GetArrOptions().length==0 && !this.bNoResults) ||
 (this.theTextBox.value.length <= ptTAObj_win5.strLastValue.length) ) {
 ptTAObj_win5.StartDelayTimeout(); } else {
 if (this.theTextBox.obj.nMaxRows==this.GetArrOptions().length)
 ptTAObj_win5.DoTypeAhead(); else {
 ptTAObj_win5.BuildList(this.theTextBox.value); }
 }
 }
 ...
 && this.getPrefixLength(this.theTextBox.value,ptTAObj_win5.strLastValue) > 0
 )) )
 {
 var bShowMatch = this.theTextBox.obj.showNoMatchMessage; this.theTextBox.obj.showNoMatchMessage = false; ptTAObj_win5.BuildList(this.theTextBox.value); this.theTextBox.obj.showNoMatchMessage = bShowMatch; if (!this.bNoResults) {
 ptTAObj_win5.strLastValue=this.theTextBox.value; return; }
 }

 if (ptTAObj_win5.theTextBox.value.length==0 || typeof ptTAObj_win5.theTextBox.value == "undefined") 
 ptTAObj_win5.HideTheBox(); else {
 this.ResetContainerEl(); ptTAObj_win5.bMadeRequest=true; ptTAObj_win5.bInProcess = true; ptTAObj_win5.TypeAhead(ptTAObj_win5.theTextBox.value); ptTAObj_win5.objLastActive=ptTAObj_win5.theTextBox; ptTAObj_win5.sQueryValue = ptTAObj_win5.theTextBox.value; ptTAObj_win5.bInProcess = false; }
 ptTAObj_win5.strLastValue=this.theTextBox.value;};function getCaretPos(focusEle)
{
...
{
 if (ptTAObj_win5.theTextBox != null && ptTAObj_win5.theTextBox != undefined)
 ptTAObj_win5.caretPos = getCaretPos(ptTAObj_win5.theTextBox); var lost = ptTAObj_win5.bLostFocus; return function () {
 ptTAObj_win5.bLostFocus = lost; lost = null; };}


PT_typeahead.prototype.TypeAhead = function(xStrText){
 if (ptTAObj_win5.bLostFocus) return;  ptTAObj_win5.SetOpValue(this.theTextBox); this.theTextBox.obj.hidden.value=this.theTextBox.id; ptTAObj_win5.bLostFocus = false;  addchg_win5(this.theTextBox);  eval(this.theTextBox.obj.serverCode); ptTAObj_win5.RestoreOpValue(this.theTextBox);};PT_typeahead.prototype.buildChoices = function (arrOptions, arrHeaders) {
 this.oWin.gFocusId = this.theTextBox.id; if (typeof this.oWin.modWin != 'undefined' && this.oWin.modWin != null) {
 return; }
 
 if (ptTAObj_win5.bLostFocus && !isFModeLayout()) {
 return;  }
 ...
 this.Close();  }
 this.BuildList(ptTAObj_win5.strLastValue ); bMadeReqiest = false; this.startPos = 0; }
 ...
 return false;  else {
 setCaretPos(this.theTextBox, ptTAObj_win5.caretPos);  if (!this.theTextBox.obj)
 ...
 var bTAFieldHasFocus = false;  if (typeof PFieldList_win5!='undefined' && PFieldList_win5) {
 for (var i = 0; i < PFieldList_win5.length; i++) {
 var fname = PFieldList_win5[i][0];  var nStop = PFieldList_win5[i][1]; for (var j=0; j<nStop; j++) { 
 var name = fname;  var nsuffix = ""; if (nStop>1) { 
 nsuffix = '$'+j;  name = fname+nsuffix; } else if (ptTAObj_win5.oDoc.getElementById(fname+'$'+j)) {
 nsuffix = '$'+j;  name = fname+nsuffix; }
 var el = ptTAObj_win5.oDoc.getElementById(name); if (el && el.getAttribute("focus") == "true") {
 ...
 if (bTAFieldHasFocus)
 return bTAFieldHasFocus; if (typeof EFieldList_win5!='undefined' && EFieldList_win5) {
 for (var i = 0; i < EFieldList_win5.length; i++) {
 var fname = EFieldList_win5[i][0];  var nStop = EFieldList_win5[i][1]; for (var j=0; j<nStop; j++) { 
 var name = fname;  var nsuffix = ""; if (nStop>1) { 
 nsuffix = '$'+j;  name = fname+nsuffix; } else if (ptTAObj_win5.oDoc.getElementById(fname+'$'+j)) {
 nsuffix = '$'+j;  name = fname+nsuffix; }
 var el = ptTAObj_win5.oDoc.getElementById(name); if (el && el.getAttribute("focus") == "true") {
 ...
 return; var theElemHeight=theElement.offsetHeight; var divgbr = null; var divgbrId = null; var divgblr = null;  var divgblrId = null; this.maxRange=3; var XYposition = ptCommonObj2.getPosition2(theElement, "win5"); selectedPosY= XYposition.y; selectedPosX= XYposition.x;   var viewportHeight = ptCommonObj2.getViewportHeight(); var nBelHeightLeft = viewportHeight + document.body.scrollTop - (selectedPosY+theElemHeight);  var nAbvHeightLeft = viewportHeight - (nBelHeightLeft+theElemHeight);  var nMinHeight = theElemHeight*3;  var nBelMaxRange = Math.floor((nBelHeightLeft - nMinHeight)/theElemHeight); var nAbvMaxRange = Math.floor((nAbvHeightLeft - nMinHeight)/theElemHeight);  var minRange = 5; nBelMaxRange = nBelMaxRange < minRange ? minRange:nBelMaxRange; nAbvMaxRange = nAbvMaxRange < minRange ? minRange:nAbvMaxRange;  if (nBelMaxRange >= nMatchLength || nBelMaxRange >= this.maxRange)
 {
 if (nBelMaxRange >= nMatchLength)
 ...
 var theElemWidth=this.theTextBox.offsetWidth; var theMatches = null; var matchGrid = null; var xPosElement = this.GetContainerEl(); this.SetMatchesArr(theText); var mLengthA= ptTAObj_win5.arrMatches.length; var boxPos = this.SetMaxRange(this.theTextBox,mLengthA);  theMatches = this.MakeMatchesArr(theText); if(theMatches.length>0) 
 matchGrid=this.CreateGrid(theMatches,mLengthA); if (matchGrid) {
 xPosElement.innerHTML=matchGrid; ptTAObj_win5.oDoc.getElementById("OptionsList_0").className="spanNormalElement"; this.currentValueSelected=-1; this.bNoResults=false; if (this.ndisplay == 0) { 
 ...
 if(this.GetArrOptions() && this.GetArrOptions().length>0){
 ...
 var sOptions = ""; mObj.setAttribute('id', this.theTextBox.id + '$typeahead'); mObj.setAttribute('sQueryValue', ptTAObj_win5.sQueryValue); var sOptions = "bPIA@1;bCache@1;sCacheParentId@pt_typeahead0;nAlign@1;bAnm@0;bPopup@1;bVertical@1;bTail@0;bResize@0;bHeader@0;bMask@0;bAutoClose@0;sStyle@ps_typeahead_modal;"; if (sOptions.indexOf("sPopupParent") == -1)
 ...
 return; var theElemHeight=theElement.offsetHeight; var theElemWidth=theElement.offsetWidth; var el = ptTAObj_win5.oDoc.getElementById("pt_typeahead0"); if (el && typeof el != 'undefined')
 el.style.display='block';  var xPosElement = this.GetContainerEl(); if('ltr' == 'ltr') 
 xPosElement.style.left = selectedPosX + "px";  else {
 var viewportWidth = ptCommonObj2.getViewportWidth();   xPosElement.style.left = "-9999px"; xPosElement.style.right = "9099px"; xPosElement.style.display='block';  var adjustWidth = 0; var ttableEle = ptTAObj_win5.oDoc.getElementById("ttable"); if (ttableEle != null)
 ...
 xPosElement.onmouseout=ptTAObj_win5.StartTimeout; xPosElement.onmouseover=ptTAObj_win5.EraseTimeout; }
 else {
 ...
 name = name.split('$')[0]; var elOP=this.oWin.document.getElementById(name+"$op"); if (elOP==null || elOP.value=="1") return; ptTAObj_win5.opValue=elOP.value; elOP.value=1; return;};PT_typeahead.prototype.RestoreOpValue = function(theTextBox) {
 var name=theTextBox.id; if (name.indexOf('$to')>0)
 name = name.split('$')[0]; var elOP=this.oWin.document.getElementById(name+"$op"); if (elOP==null) return; if (ptTAObj_win5.opValue != -1) {
 elOP.value = ptTAObj_win5.opValue; ptTAObj_win5.opValue = -1; }
...
if (!ptCommonObj2.isSearchPage(this.oForm)) return false; var name=theTextBox.id; if (name.indexOf('$to')>0)
 ...
 this.countForId=0; this.currentTotalRow=0;  ptTAObj_win5.arrMatches=new Array(); this.DecodeArrOptions(); for(var i=0;i<this.GetArrOptions().length;i++) {
 var str = this.GetArrOptions()[i][this.theTextBox.obj.colnum]; var theMatch = this.getPrefixLength(str, xCompareStr)>0; if (theMatch) {
 ptTAObj_win5.arrMatches[ptTAObj_win5.arrMatches.length]=this.GetArrOptions()[i]; }
 }
...
};PT_typeahead.prototype.getIndex = function(str, prefix) 
{
 ...
 this.countForId=0; this.currentTotalRow=0; var matchArray=new Array(); for(var i=this.startPos;(i<ptTAObj_win5.arrMatches.length && (i-this.startPos)<this.maxRange);i++){
 var matchRowArr=new Array(); for (var j=0;j<this.GetArrOptions()[i].length;j++) {
 if (j==this.theTextBox.obj.colnum) {
 matchRowArr[matchRowArr.length]=this.CreateBold(ptTAObj_win5.arrMatches[i][j],xCompareStr,i); }
 else 
 matchRowArr[matchRowArr.length]=this.CreateBoldOtherCol(ptTAObj_win5.arrMatches[i][j],i); }
 ...
 this.startPos = 0; this.BuildList(ptTAObj_win5.strLastValue); var xPosElement = this.GetContainerEl(); xPosElement.onmouseover(); this.MoveHighlight(1);};PT_typeahead.prototype.ScrollDown = function(){
 this.startPos = this.startPos + this.maxRange-1; if (this.startPos < ptTAObj_win5.arrMatches.length)
 this.BuildList(ptTAObj_win5.strLastValue); var xPosElement = this.GetContainerEl(); xPosElement.onmouseover(); this.MoveHighlight(1);};PT_typeahead.prototype.CreateGrid = function(matchArray,mLengthA) {
 ...
 var sCloseTAResult = "<img align='right' src='"+this.url_close+"'/ onclick='ptTAObj_win5.HideTheBox()'>"; matchGrid=matchGrid+this.trStart; matchGrid=matchGrid+sCloseTAResult;  matchGrid=matchGrid+this.trEnd; }
 
 if( isTouchDevice() ){
 var sScrollUp = "<img align='absmiddle' src='"+this.url_up+"'/ onclick='ptTAObj_win5.ScrollUp()'>"; } else {
 var sScrollUp = "<img align='absmiddle' src='"+this.url_up+"'/ onmouseover='ptTAObj_win5.ScrollUp()'>"; } 
 ...
 if (((this.startPos+this.maxRange) < ptTAObj_win5.arrMatches.length) && mLength>=this.maxRange){
 if( isTouchDevice() ){
 var sScrollDown = "<img align='absmiddle' src='"+this.url_dn+"'/ onclick='ptTAObj_win5.ScrollDown()'>"; } else {
 var sScrollDown = "<img align='absmiddle' src='"+this.url_dn+"'/ onmouseover='ptTAObj_win5.ScrollDown()'>"; }
 ...
 matchGrid += this.tableEnd; matchGrid += "</div>"; matchGrid += "<div class='ps_box-grid ps_box-grid-typeahead ps_scrollable sbar sbar_v ps_scrollable_v' id='" + getPageFormName(this.theTextBox) + "div" + this.theTextBox.id + "grid' onscroll='ptTAObj_win5.EraseTimeout();' onmouseup=\"setFocus_win5('"+this.theTextBox.id+"');\">"; matchGrid += this.tableStart;  if (arrHeaders.length > 0) {
 ...
 matchGrid += "<tr class='ps_grid-row' onclick='ptTAObj_win5.SetText(" + i + ");typeAheadPositionTopClear();'>";  for (var j=0;j<matchArray[i].length;j++){
 ...
 matchGrid += "</tbody>";  matchGrid += this.tableEnd + "</div>"; matchGrid += "</div>"; return matchGrid;};PT_typeahead.prototype.getPrefixLength = function(str, prefix) 
{
 ...
 if (newValue == this.currentTotalRow && ((this.startPos+this.maxRange) < ptTAObj_win5.arrMatches.length)){
 this.ScrollDown(); this.currentValueSelected=0; ptTAObj_win5.GrabHighlighted(); this.MoveHighlight(xDir); }
 if (newValue == -1 && this.startPos>0){
 this.ScrollUp(); this.currentValueSelected=this.currentTotalRow; ptTAObj_win5.GrabHighlighted(); this.MoveHighlight(xDir); }
 }
 ...
 var odd=false; var totalCol=ptTAObj_win5.arrMatches[0].length; for(i=0;i<this.currentTotalRow;i++){
 var tObj = ptTAObj_win5.oDoc.getElementById('OptionsList_'+i*totalCol); var trObj = tObj.parentNode.parentNode; for (var j=0; j<trObj.childNodes.length; j++) {
 if (isFModeLayout()) {
 ...
 if (theTextBox) {
 var selectOBJ=ptTAObj_win5.oDoc.getElementById('OptionsList_'+this.currentValueSelected); this.currentValueSelected = selectOBJ.getAttribute("theArrayNumber") - this.startPos;  }
 else
 var selectOBJ=ptTAObj_win5.oDoc.getElementById('OptionsList_'+this.currentValueSelected*totalCol);  var trObj = selectOBJ.parentNode.parentNode; if (isFModeLayout()) {
 ...
 var bUnique = true;  var selectedValue; var aboveValue; var belowValue;  selectedValue = this.DecodeValue(ptTAObj_win5.arrMatches[xVal][this.theTextBox.obj.colnum]); if (xVal > 0) 
 aboveValue = this.DecodeValue(ptTAObj_win5.arrMatches[xVal -1][this.theTextBox.obj.colnum]); if (xVal + 1 < ptTAObj_win5.arrMatches.length) 
 belowValue= this.DecodeValue(ptTAObj_win5.arrMatches[xVal + 1][this.theTextBox.obj.colnum]); if (selectedValue == aboveValue || selectedValue == belowValue)
 bUnique = false; return bUnique; };PT_typeahead.prototype.GetIndexBasedOnSelectedVal = function(xVal) {
 var index = 0; var i = xVal; var aboveValue; var selectedValue = this.DecodeValue(ptTAObj_win5.arrMatches[xVal][this.theTextBox.obj.colnum]); while (i > 0) {
 aboveValue = this.DecodeValue(ptTAObj_win5.arrMatches[i -1][this.theTextBox.obj.colnum]); if (selectedValue == aboveValue) 
 i--; else
 break; }
 index = xVal - i; return index;};PT_typeahead.prototype.UpdateControlField = function(xVal) {
 if (ptCommonObj2.isSearchPage(this.oForm)) return;  var rowNum = this.GetIndexBasedOnSelectedVal(xVal);  var val = '#ICRow' + rowNum; document.win5.target = window.id; document.win5.ICAction.value = val; aAction_win5(document.win5, val);};PT_typeahead.prototype.SetText = function(xVal){
 if ((this.theTextBox).focus)
 (this.theTextBox).focus(); else if ((this.theTextBox).setActive)
 (this.theTextBox).setActive(); this.theTextBox.value=this.DecodeValue(ptTAObj_win5.arrMatches[xVal][this.theTextBox.obj.colnum]);  if (!ptTAObj_win5.IsSelectedUnique(xVal)) {
 
 var postUrl_win5_old = postUrl_win5; postUrl_win5 += (postUrl_win5.split("?")[1] ? "&":"?") + "ICTypeAheadSelectedUnique=0"; this.UpdateControlField(xVal); postUrl_win5 = postUrl_win5_old; } 
 addchg_win5(this.theTextBox);   var thisObj = this.theTextBox.attributes.getNamedItem('onchange'); if (thisObj != null && thisObj.nodeValue != null) 
 this.theTextBox.onchange();  if( isTouchDevice() ){
 ptTAObj_win5.HideTheBox();  } 
 ...
 if(this.currentValueSelected>=0){
 this.bGrabHighlighted = true; var xVal = this.currentValueSelected+this.startPos;  this.theTextBox.value=this.DecodeValue(ptTAObj_win5.arrMatches[xVal][this.theTextBox.obj.colnum]); }
};PT_typeahead.prototype.AutoWinAdj = function(xPosElement){
 ptTAObj_win5.bodyScrollLeft=ptTAObj_win5.oDoc.body.scrollLeft;  ptTAObj_win5.bodyScrollTop=ptTAObj_win5.oDoc.body.scrollTop; var t1=parseInt(xPosElement.offsetHeight,10); var t2=parseInt(document.body.offsetHeight,10)-(parseInt(xPosElement.style.top,10)-ptTAObj_win5.oDoc.body.scrollTop); if (t1>t2)
 ptTAObj_win5.oDoc.body.scrollTop=t1-t2+10; t1=parseInt(xPosElement.offsetWidth,10); t2=parseInt(document.body.offsetWidth,10)-(parseInt(xPosElement.style.left,10)-ptTAObj_win5.oDoc.body.scrollLeft); if (t1>t2) {
 if (ptTAObj_win5.oDoc.win5.ICActionPrompt.value=='true') {
 var o=ptTAObj_win5.oWin.parent.document.getElementById('popupContainer'); ptTAObj_win5.promptWidth = Math.max(0,o.style.width.replace('px','')); var newW=ptTAObj_win5.promptWidth+t1-t2+20+'px'; o.style.width = newW; o=ptTAObj_win5.oWin.parent.document.getElementById('popupFrame'); o.style.width = newW; o=ptTAObj_win5.oWin.parent.document.getElementById("popupTitleBar"); o.style.width = newW; }
 ...
 ptTAObj_win5.oDoc.body.scrollLeft=selectedPosX-20; else
 ptTAObj_win5.oDoc.body.scrollLeft=t1-t2+20; }
};PT_typeahead.prototype.AutoWinUnAdj = function(){
 ptTAObj_win5.oDoc.body.scrollLeft=ptTAObj_win5.bodyScrollLeft;  ptTAObj_win5.oDoc.body.scrollTop=ptTAObj_win5.bodyScrollTop;  if (!ptTAObj_win5.oDoc.win5.ICActionPrompt || ptTAObj_win5.oDoc.win5.ICActionPrompt.value!='true') {
 ptTAObj_win5.promptWidth=0;return; }
 if (ptTAObj_win5.promptWidth==0) return; var o=ptTAObj_win5.oWin.parent.document.getElementById('popupContainer'); o.style.width = ptTAObj_win5.promptWidth+'px'; o=ptTAObj_win5.oWin.parent.document.getElementById('popupFrame'); o.style.width = ptTAObj_win5.promptWidth+'px'; o=ptTAObj_win5.oWin.parent.document.getElementById("popupTitleBar"); o.style.width = ptTAObj_win5.promptWidth+'px'; };PT_typeahead.prototype.ClearResultData = function() {
 ptTAObj_win5.arrHeaders.length = 0; this.GetArrOptions().length = 0; ptTAObj_win5.arrMatches.length = 0;};PT_typeahead.prototype.DecodeArrOptions = function() {
 ...
 this.bGrabHighlighted = false; var el=ptTAObj_win5.oDoc.getElementById('pt_typeahead0'); if (el && typeof el != 'undefined')
 el.style.display='none'; ptTAObj_win5.oDoc.getElementById('pt_typeahead').style.display='none'; ptTAObj_win5.currentValueSelected=-2; ptTAObj_win5.EraseTimeout();    this.startPos = 0;  ptTAObj_win5.ClearResultData(); };PT_typeahead.prototype.IsGrabHighlighted = function() { 
 if (this.bGrabHighlighted)
 return true; return false;};PT_typeahead.prototype.EraseDelayTime = function(){
 clearTimeout(ptTAObj_win5.isDelayTiming); ptTAObj_win5.isDelayTiming=false;};PT_typeahead.prototype.StartDelayTimeout = function(){
 if (!ptTAObj_win5.theTextBox) return; ptTAObj_win5.isDelayTiming=setTimeout("ptTAObj_win5.DoTypeAhead()",this.nDelayTime);};PT_typeahead.prototype.EraseDelayTimeout = function(){
 clearTimeout(ptTAObj_win5.isDelayTiming); ptTAObj_win5.isDelayTiming=false;};PT_typeahead.prototype.EraseTimeout = function(){
 clearTimeout(ptTAObj_win5.isTiming); ptTAObj_win5.isTiming=false;};PT_typeahead.prototype.StartTimeout = function(){
 if (!ptTAObj_win5.theTextBox) 
 return; ptTAObj_win5.isTiming=setTimeout("ptTAObj_win5.HideTheBox()",ptTAObj_win5.theTextBox.obj.theVisibleTime);};PT_typeahead.prototype.GetContainerEl = function () { 
 var xPosElement = null; if (isFModeLayout()) {
 xPosElement = ptTAObj_win5.oDoc.getElementById(this.theTextBox.id + '$typeahead'); if (!xPosElement)
 xPosElement = ptTAObj_win5.oDoc.getElementById("pt_typeahead"); }
 else
 xPosElement = ptTAObj_win5.oDoc.getElementById("pt_typeahead"); return xPosElement;};PT_typeahead.prototype.ResetContainerEl = function () {
...
function typeAheadPositionTopClear() {
 ptTAObj_win5.Close(); ptTAObj_win5.PositionTopClear();}


function isTypeHeadeNew() {
 return ptTAObj_win5.isNew();}
